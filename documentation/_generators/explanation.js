...
A(P("كل الـ models الخمسة بتستخدم transfer learning: بدل ما تتعلّم الرؤية من الصفر، بتبدأ من weights مدرّبة على ImageNet (dataset ضخم لصور طبيعية) وبعدين بنعمل fine-tune على أشعة الصدر. ده standard في الـ medical imaging لأن الـ datasets الطبية صغيّرة، والـ low-level features (الحواف والـ textures) بتنتقل كويس."));
...
A(P("Faster R-CNN بيتدرّب على الـ TorchVision training loop: optimizer = SGD (momentum 0.9)، learning-rate schedule = linear warmup بعدها cosine decay، تجميد الـ backbone في أول epoch عشان الـ heads الجديدة تستقر قبل ما نعمل fine-tune للشبكة كلها، AMP، early stopping على mAP@0.5، وbest-checkpoint restore. بيتحسّن على الـ Faster R-CNN multi-task loss (objectness + box للـ RPN، وclassification + box-regression للـ head). وقت الـ inference بنطبّق Non-Maximum Suppression (NMS) عشان نشيل الـ boxes المكررة المتداخلة (مشروح في القسم B). أما YOLOv8 فبيتدرّب من خلال الـ Ultralytics pipeline بالـ augmentation الجاهز بتاعه."));
...
A(P([t("ملاحظة: ", { bold: true }), t("كل الصور دي نواتج حقيقية من الـ web application الشغّال. الـ heatmaps في حالة الالتهاب بتركّز على الرئة المصابة، وده بيأكد إن الـ models بتتعلّم إشارات طبية حقيقية مش artifacts.")]));
...
A(bullet([t("Model registry lookup: ", { bold: true }), t("بيدوّر على الـ model في registry (جدول بيربط مفتاح الـ model بالـ weights والـ task والـ thresholds). الـ registry فيه دلوقتي 5 models: YOLO، Faster R-CNN (detection)، وResNet50، DenseNet121، EfficientNet-B0 (classification).")]));
