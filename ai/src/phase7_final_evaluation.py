...
def run_phase7_final_evaluation():
    ...
    for m in models:
        ...
        metric = "mAP@0.5" if m in {"yolo", "fasterrcnn"} else "AUC"
    ...
    det_rows = [r for r in rows if r.model in {"yolo", "fasterrcnn"}]
    cls_rows = [r for r in rows if r.model not in {"yolo", "fasterrcnn"}]
